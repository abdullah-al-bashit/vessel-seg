...
\caption{Attention U-Net architecture. A three-channel input (grayscale intensity $I$, gradient magnitude $G$, sharpness $S$) is encoded by a pretrained trainable ResNet34 backbone that produces five feature maps at progressively coarser spatial scales ($e_0$--$e_4$). Attention gates (Attn0--Attn3) filter each encoder skip connection using the corresponding decoder query before the skip is fused with the upsampled decoder state. A dilated refinement head ($d=2$, effective $9\times9$ receptive field) follows the last decoder stage and a $1\times1$ projection with sigmoid activation produces the final vessel-probability map.}
...
\begin{axis}[
  ...
  legend style={font=\tiny, cells={anchor=west}, row sep=-2pt},
  grid=both, grid style={gray!20},
  ymin=0
]
\addplot[MidnightBlue,  thick, dashed, forget plot] table[x=epoch,y=train_loss]{figures/plots/fold1_curves.dat};
\addplot[MidnightBlue,  thick]                      table[x=epoch,y=val_loss  ]{figures/plots/fold1_curves.dat};
\addlegendentry{Fold 1}
\addplot[RedOrange,     thick, dashed, forget plot] table[x=epoch,y=train_loss]{figures/plots/fold2_curves.dat};
\addplot[RedOrange,     thick]                      table[x=epoch,y=val_loss  ]{figures/plots/fold2_curves.dat};
\addlegendentry{Fold 2}
\addplot[ForestGreen,   thick, dashed, forget plot] table[x=epoch,y=train_loss]{figures/plots/fold3_curves.dat};
\addplot[ForestGreen,   thick]                      table[x=epoch,y=val_loss  ]{figures/plots/fold3_curves.dat};
\addlegendentry{Fold 3}
\addplot[Purple,        thick, dashed, forget plot] table[x=epoch,y=train_loss]{figures/plots/fold4_curves.dat};
\addplot[Purple,        thick]                      table[x=epoch,y=val_loss  ]{figures/plots/fold4_curves.dat};
\addlegendentry{Fold 4}
\addplot[TealBlue,      thick, dashed, forget plot] table[x=epoch,y=train_loss]{figures/plots/fold5_curves.dat};
\addplot[TealBlue,      thick]                      table[x=epoch,y=val_loss  ]{figures/plots/fold5_curves.dat};
\addlegendentry{Fold 5}
\end{axis}
\end{tikzpicture}
\caption{Loss (solid = val, dashed = train)}
...
\addplot[MidnightBlue,  thick, dashed, forget plot] table[x=epoch,y=train_dice]{figures/plots/fold1_curves.dat};
\addplot[MidnightBlue,  thick]                      table[x=epoch,y=val_dice  ]{figures/plots/fold1_curves.dat};
\addlegendentry{Fold 1}
\addplot[RedOrange,     thick, dashed, forget plot] table[x=epoch,y=train_dice]{figures/plots/fold2_curves.dat};
\addplot[RedOrange,     thick]                      table[x=epoch,y=val_dice  ]{figures/plots/fold2_curves.dat};
\addlegendentry{Fold 2}
\addplot[ForestGreen,   thick, dashed, forget plot] table[x=epoch,y=train_dice]{figures/plots/fold3_curves.dat};
\addplot[ForestGreen,   thick]                      table[x=epoch,y=val_dice  ]{figures/plots/fold3_curves.dat};
\addlegendentry{Fold 3}
\addplot[Purple,        thick, dashed, forget plot] table[x=epoch,y=train_dice]{figures/plots/fold4_curves.dat};
\addplot[Purple,        thick]                      table[x=epoch,y=val_dice  ]{figures/plots/fold4_curves.dat};
\addlegendentry{Fold 4}
\addplot[TealBlue,      thick, dashed, forget plot] table[x=epoch,y=train_dice]{figures/plots/fold5_curves.dat};
\addplot[TealBlue,      thick]                      table[x=epoch,y=val_dice  ]{figures/plots/fold5_curves.dat};
\addlegendentry{Fold 5}
\end{axis}
\end{tikzpicture}
\caption{Dice (solid = val, dashed = train)}
\label{fig:dice-curves}
\end{subfigure}
\caption{Training and validation curves across all five folds, parsed from \nolinkurl{train_6670898.out}. Solid lines show validation metrics; dashed lines of the same colour show training metrics. Fold~1 (blue) reached the lowest validation loss and was selected for final held-out inference. The narrow training–validation gap across all folds indicates that the model generalises rather than memorising tile-level patterns.}
